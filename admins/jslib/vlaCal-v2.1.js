...
var vlaCalendar = new Class({
	...
	initialize: function(_container, _options) {
		...
		this.u('base', pars, function() { 
			...
			if(_class.picker) {
				...
				else if(_class.prefillDate == true) _class.pick(NIK.decode(_class.label.getProperty('date')));
			}
		}, _container);
	},
	...
	initializeCalendarFunctions: function() {
		...
		var vars = NIK.decode(this.mainLoader.getElement('table').getProperty('summary'));
		var _class = this; 
				...
				if(this.picker) {
					clickables.each(function(_clickable) {
						_clickable.onclick = function() { 
							_class.pick(NIK.decode(_clickable.getProperty('date')));
