...
var NIK = new Hash({
	...
	encode: function(obj){
		switch ($type(obj)){
			case 'string':
				return '"' + obj.replace(/[\x00-\x1f\\"]/g, NIK.$replaceChars) + '"';
			case 'array':
				return '[' + String(obj.map(NIK.encode).filter($defined)) + ']';
			case 'object': case 'hash':
				var string = [];
				Hash.each(obj, function(value, key){
					var json = NIK.encode(value);
					if (json) string.push(NIK.encode(key) + ':' + json);
				});
				return '{' + string + '}';
	...
	$replaceChars: function(chr){
		return NIK.$specialChars[chr] || '\\u00' + Math.floor(chr.charCodeAt() / 16).toString(16) + (chr.charCodeAt() % 16).toString(16);
	},
...
/*
Script: Request.NIK.js
	Extends the basic Request Class with additional methods for sending and receiving NIK data.

License:
	MIT-style license.
*/

Request.NIK = new Class({
	...
	initialize: function(options){
		this.parent(options);
		this.headers.extend({'Accept': 'application/json', 'X-Request': 'NIK'});
	},

	success: function(text){
		this.response.json = NIK.decode(text, this.options.secure);
